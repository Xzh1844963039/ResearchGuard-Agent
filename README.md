...
当前工程重点已经从早期迁移代码，推进到真实论文解析、section-aware chunking、Embedding + Persistent Indexing v1、Retrieval v1、Chroma Vector Database Backend v1、Reranker v1、Query Rewrite v1 和 Evidence Sufficiency v1 验收阶段。Parser v5 已完成严格验收，结论为 `PASS_WITH_CHUNK_FIXES`；Chunking v1 已完成最终边界修复，当前为 `APPROVED_FOR_INDEXING`；Indexing v1、Retrieval v1、Chroma v1、Reranker v1 和 Query Rewrite v1 均通过独立验证。Evidence Sufficiency v1 已接在最终 Top-k evidence 之后，以结构化 LLM Judge 判断 `strong/partial/unsupported`，44 条 benchmark hard checks 全部为 0，no-answer false positive rate 从 retrieval-only 的 `1.0` 降到 `0.0`，结论为 `PASS`。Retrieval 当前默认使用 Chroma dense backend；reranker、query rewrite 和 evidence check 默认关闭，可通过 CLI 显式启用。Agentic RAG、answer generation、claim extraction 和 citation audit 仍未接入新主流程。
...
论文 PDF
  ...
  -> supported: allow future answer generation
  -> partial / unsupported: reject or request more evidence
  -> Agentic RAG answer（尚未实现）
  -> claim extraction
  -> evidence audit
...
当前已经重点完成的是 `PDF -> layout/block/section -> section-aware chunk -> index_v1 -> query rewrite/multi-query -> NumPy/Chroma dense backend -> Dense + BM25 + RRF -> Cross-Encoder rerank -> Evidence Sufficiency` 这段底座。后续 Agentic RAG、answer generation、claim extraction、citation audit 和产品化 API/UI 还需要继续接入。
...
- 在 RAG 中的作用：作为未来 answer generation 前的 gate；当前只输出 verdict，不生成答案、不提取 claim、不做 citation audit，也不能证明整个 corpus 中绝对没有答案。
...
researchguard/retrieval/      Retrieval/Rewrite/Reranker/Evidence Judge：Dense、BM25/RRF、multi-query、Cross-Encoder、sufficiency、cache
...
configs/                      parser/index/retrieval/Chroma/reranker/query rewrite/evidence sufficiency 等配置
data/eval/                    冻结 Retrieval benchmark 与独立 Evidence Sufficiency benchmark
...
| Agentic RAG | 已迁移但未重构 | legacy 模块存在，尚未接入当前 Retrieval/Reranker/Evidence Judge 主流程；当前没有 answer generation、claim extraction 或 retry workflow。 |
| Evidence Audit | 部分实现 | `researchguard/audit` 中已有 claim/evidence/audit 相关模块，但未与新主流程产品化串联。 |
| Evaluation | 部分实现 | Retrieval、Reranker、Query Rewrite 和 Evidence Sufficiency 已有独立 benchmark/validation；answer / agentic / citation audit 评测尚未完成。 |
...
- Indexing v1、Retrieval v1、Reranker v1、Query Rewrite v1 和 Evidence Sufficiency v1 已完成后，五篇语料继续作为 answer generation 和 evidence audit 的固定回归集。
...
### 13.9 Chroma build 和 incremental sync
...
### 13.11 Retrieval / Rewrite / Reranker / Evidence CLI
...
### 13.14 Query Rewrite validation
...
### 13.15 Evidence Sufficiency validation
...
- Retrieval/Query Rewrite/Reranker/Evidence Sufficiency 已形成候选排序和 answerability gate 主流程，但仍不是端到端问答。
...
- Query Rewrite 本身不判断 evidence sufficiency；该职责现在由检索后的独立 Judge 承担。当前仍未实现 neighbor expansion、answer generation、claim extraction、citation audit 或 Agentic RAG workflow。
...
当前 Retrieval v1、Chroma v1、Reranker v1、Query Rewrite v1 和 Evidence Sufficiency v1 均已通过独立验证。下一阶段应围绕 answer gating 后的受控 Answer Generation 展开，但不能跳过独立 hold-out 和 provenance 设计。

1. 冻结 Evidence Sufficiency v1 的公开 schema、prompt/config version、44-query regression benchmark 和 validation outputs。
2. 新增与当前开发 benchmark 分离的 answerable/no-answer/partial blind hold-out；扩大 no-answer 类型和论文数量，避免用当前 4 条 unrelated query 估计生产 FPR。
3. 对 8 个 false negatives 做 retrieval miss 与 Judge conservatism 分解；优先改进 filter/query-intent 传递和 evidence coverage，不按 query ID、固定文本或 benchmark labels 写特殊分支。
4. 设计 Answer Generation v1：只有 `strong + answerable=true` 才允许生成；partial/unsupported 和 Judge fallback 必须拒答或明确请求更多 evidence。
5. Answer Generation 必须保留 original/normalized/expansion、RRF/rerank、supporting chunk、page/section/source block provenance；不得把 overlap 或重复召回当作独立证据。
6. 将 claim extraction、claim-evidence alignment、citation audit 和 evidence chain graph 保留为后续独立阶段，不与第一版 answer generation 混做。
7. 语料、embedding、Chroma、reranker、rewrite model 或 Judge prompt/config 变化时，重新运行 Retrieval、Reranker、Query Rewrite 和 Evidence Sufficiency 全部回归。

## 17. Development Documentation Rule
