...
当前工程重点已经从早期迁移代码，推进到真实论文解析、section-aware chunking、Embedding + Persistent Indexing v1、Retrieval v1 和 Chroma Vector Database Backend v1 验收阶段。Parser v5 已完成严格验收，结论为 `PASS_WITH_CHUNK_FIXES`；Chunking v1 已完成最终边界修复，当前为 `APPROVED_FOR_INDEXING`；Indexing v1 已完成真实 OpenAI embedding、本地 NumPy dense/BM25 sparse index 和严格验证，结论为 `PASS`；Retrieval v1 已完成 dense、BM25、RRF hybrid、metadata filters 和 40 条 query benchmark，结论为 `PASS`；Chroma v1 已复用现有 337 个 embeddings 建立本地持久化 collection，并通过完整性、持久化、过滤、增量和 NumPy 搜索一致性验证，结论为 `PASS`。Retrieval v1 当前默认使用 Chroma dense backend，同时保留 NumPy 精确基线和显式回退。Agentic RAG、answer generation、LLM rerank、query rewrite、evidence sufficiency 和 citation audit 仍未接入新主流程。
...
当前已经重点完成的是 `PDF -> layout/block/section -> section-aware chunk -> index_v1 -> NumPy/Chroma dense backend -> retrieval_v1` 这段底座。后续 Agentic RAG、answer generation、evidence audit 和产品化 API/UI 还需要继续接入。
...
researchguard/retrieval/      Retrieval v1：NumPy/Chroma dense backend、BM25/hybrid、metadata filters、result schema
...
configs/                      parser/index/retrieval/Chroma 等配置
...
### 10.9 Chroma build 和 incremental sync
...
1. 冻结 Retrieval v1 与 Chroma v1 的 backend interface、result schema、benchmark 和 validation reports。
...
## 14. Development Documentation Rule
