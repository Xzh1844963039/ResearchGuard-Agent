...
当前 Evidence Engine 主流程已经完成本地集成与验证；v2 在其上增加了 Tool Facade、受约束的单 Agent Controller、Scholarly Discovery、Research Workflow Skills，以及 Research Memory 与 Evidence Ledger。历史 Agent 和早期 Audit 代码仍保留用于兼容；历史 `MemoryStore` 也继续服务迁移代码，但不承担当前 Controller 的长期科研记忆。
...
- Research Memory 只支持结构化 run、evidence 和 failure 查询；没有聊天历史、用户画像、偏好学习、跨任务自动上下文注入或 embedding retrieval。
...
5. 在独立 Agent/workflow benchmark 上评估 task classification、workflow trace、policy stop、论文识别、memory lineage 和失败恢复。
...
9. 评估是否需要受约束的短期 session context；在有 benchmark 前不引入用户画像、Multi-Agent 或开放式自治循环。
