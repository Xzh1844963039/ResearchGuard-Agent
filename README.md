...
当前工程重点已经从早期迁移代码，推进到真实论文解析、section-aware chunking、Embedding + Persistent Indexing v1、Retrieval v1、Chroma Vector Database Backend v1 和 Reranker v1 验收阶段。Parser v5 已完成严格验收，结论为 `PASS_WITH_CHUNK_FIXES`；Chunking v1 已完成最终边界修复，当前为 `APPROVED_FOR_INDEXING`；Indexing v1、Retrieval v1 和 Chroma v1 均通过独立验证；Reranker v1 已使用真实 Cross-Encoder 对 Hybrid RRF 的 Top-20 候选精排到 Top-10，40 条 query 正式对比中 hard checks 全部为 0，`Recall@10`、`MRR@10` 和 `nDCG@10` 均提升，结论为 `PASS`。Retrieval 当前默认使用 Chroma dense backend，并保留 NumPy 精确基线、显式 backend 回退和 `--no-rerank` 回退。Agentic RAG、answer generation、query rewrite、evidence sufficiency、answerability detection 和 citation audit 仍未接入新主流程。
...
当前已经重点完成的是 `PDF -> layout/block/section -> section-aware chunk -> index_v1 -> NumPy/Chroma dense backend -> Dense + BM25 + RRF -> Cross-Encoder rerank` 这段底座。后续 Query Rewrite、Evidence Sufficiency、Agentic RAG、answer generation、evidence audit 和产品化 API/UI 还需要继续接入。
...
researchguard/retrieval/      Retrieval/Reranker：NumPy/Chroma dense、BM25/RRF、Cross-Encoder、cache、统一 result schema
...
configs/                      parser/index/retrieval/Chroma/reranker 等配置
data/eval/                    Retrieval/Reranker 共用的 40 条 benchmark queries
...
- Indexing v1、Retrieval v1 和 Reranker v1 已完成后，五篇语料继续作为 Query Rewrite、Evidence Sufficiency、answer generation 和 evidence audit 的固定回归集。
...
存放 Retrieval v1 与 Reranker v1 共用的 40 条人工可核对 benchmark queries。`relevant_chunk_ids` 必须来自当前 `data/indexes/index_v1/corpus_manifest.jsonl`；Reranker 验收禁止修改这些 labels。
...
### 11.9 Chroma build 和 incremental sync
...
- Retrieval/Reranker 已形成独立 evidence candidate ranking 主流程和 benchmark，但仍不是端到端问答。
...
- Retrieval/Reranker 不包含 query rewrite、evidence sufficiency、answerability detection、retry、neighbor expansion、answer generation、citation audit 或 Agentic RAG。
...
当前 Retrieval v1、Chroma v1 和 Reranker v1 均已通过独立验证。下一步建议冻结当前 ranking baseline，再选择 Query Rewrite v1 或 Evidence Sufficiency 作为独立阶段；从当前 `no_answer_false_positive_rate=1.0` 看，Evidence Sufficiency / Answerability Detection 的产品价值更直接。

1. 冻结 Retrieval v1、Chroma v1 与 Reranker v1 的 backend interface、result schema、benchmark 和 validation reports。
...
3. 也可先实现 Query Rewrite v1，但必须继续保留原 query 与 rewritten query 的独立 trace 和 baseline 对比。
...
6. 语料扩大或 Chroma 版本/配置变化时重新运行 NumPy parity、filter 和 persistence 验收。
7. 继续评估 caption/table/equation 的复杂视觉语义绑定，尤其是跨栏、跨页和图文环绕场景。
8. 复核剩余 `<150` chunk 和 paper_agent 的 heading_path 缺失场景，但不得为了检索指标擅自修改已冻结的 `chunk_eval_v1`。

## 15. Development Documentation Rule
