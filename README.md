...
Phase 6 在 `researchguard/tracing/` 建立版本化 `AgentTrace`，统一记录 query、plan、workflow、Tool calls、observations、canonical evidence、answer、citation audit、memory context、memory snapshot 和最终状态。Trace 可直接序列化为 JSON，也可由 formatter 转换为 CLI Markdown；它是只读观测层，不参与 Controller stop decision。
