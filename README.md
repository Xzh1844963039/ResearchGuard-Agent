...
当前工程重点已经从早期迁移代码，推进到真实论文解析、section-aware chunking、Embedding + Persistent Indexing v1、Retrieval v1、Chroma Vector Database Backend v1、Reranker v1、Query Rewrite v1、Evidence Sufficiency v1 和 Answer Generation v1 验收阶段。Parser v5 已完成严格验收，结论为 `PASS_WITH_CHUNK_FIXES`；Chunking v1 已完成最终边界修复，当前为 `APPROVED_FOR_INDEXING`；Indexing v1、Retrieval v1、Chroma v1、Reranker v1、Query Rewrite v1 和 Evidence Sufficiency v1 均通过独立验证。Answer Generation v1 只消费 Evidence Judge 标记为 `strong` 的 supporting chunks，并为 `partial/unsupported` 固定拒答；22 条 benchmark 的 8 项 hard checks 全部为 0，结论为 `PASS`。Retrieval 当前默认使用 Chroma dense backend；reranker、query rewrite、evidence check 和 answer generation 默认关闭，可通过 CLI 显式启用。Claim extraction、claim verification、citation audit、Agent workflow 和产品化 API/UI 仍未接入新主流程。
...
当前已经重点完成的是 `PDF -> layout/block/section -> section-aware chunk -> index_v1 -> query rewrite/multi-query -> NumPy/Chroma dense backend -> Dense + BM25 + RRF -> Cross-Encoder rerank -> Evidence Sufficiency -> Evidence-grounded Answer Generation` 这段主链路。后续 claim extraction、claim verification、citation audit、Agent workflow 和产品化 API/UI 还需要继续接入。
...
- 输入：计划使用当前 Retrieval/Evidence/Answer 主链路的结构化结果和 provenance。
- 输出：尚未正式接入新主流程。
- 当前状态：旧 agent、audit、evaluation 模块存在，但尚未围绕新主链路重构成正式 Agentic RAG / claim verification / citation audit pipeline。
...
researchguard/retrieval/      Retrieval/Rewrite/Reranker/Evidence/Answer：检索、融合、精排、充分性 gate、受控生成与 cache
...
configs/                      parser/index/retrieval/Chroma/reranker/rewrite/evidence/answer generation 等配置
data/eval/                    Retrieval、Evidence Sufficiency 与 Answer Generation 独立 benchmark
...
| Agentic RAG | 已迁移但未重构 | legacy 模块存在，尚未接入当前 Retrieval/Reranker/Evidence/Answer 主流程；当前没有 claim extraction、tool retry 或 Agent workflow。 |
| Evidence Audit | 部分实现 | `researchguard/audit` 中已有 claim/evidence/audit 相关模块，但未与新主流程产品化串联。 |
| Evaluation | 部分实现 | Retrieval、Reranker、Query Rewrite、Evidence Sufficiency 和 Answer Generation 已有独立 benchmark/validation；agentic / citation audit 评测尚未完成。 |
...
- Indexing v1、Retrieval v1、Reranker v1、Query Rewrite v1、Evidence Sufficiency v1 和 Answer Generation v1 已完成后，五篇语料继续作为未来 citation audit 的固定回归集。
...
### 14.9 Chroma build 和 incremental sync
...
### 14.11 Retrieval / Rewrite / Reranker / Evidence / Answer CLI
...
### 14.14 Query Rewrite validation
...
### 14.15 Evidence Sufficiency validation
...
### 14.16 Answer Generation validation
...
- Retrieval/Query Rewrite/Reranker/Evidence Sufficiency/Answer Generation 已形成受控问答主流程，但仍不是完整 Agentic RAG 或 Evidence Audit 产品。
...
- Query Rewrite 本身不判断 evidence sufficiency；该职责由检索后的独立 Judge 承担。当前仍未实现 neighbor expansion、claim extraction、claim verification、citation audit 或 Agentic RAG workflow。
...
- 当前 citation 只保证 ID 与 `doc_id/section/page` provenance 来自允许 evidence，不保证每条 citation 对答案各句的 entailment、citation completeness 或最小充分性；这些属于后续独立 Citation Audit。
...
当前 Retrieval v1、Chroma v1、Reranker v1、Query Rewrite v1、Evidence Sufficiency v1 和 Answer Generation v1 均已通过独立验证。下一阶段应单独进入 Citation Audit / Claim Verification，不能把当前 citation provenance validation 当成语义审计已经完成。

1. 冻结 Answer Generation v1 的公开 schema、strong gate、prompt/config version、22-query benchmark 和 validation outputs。
2. 新增与开发 benchmark 分离的 blind hold-out，覆盖跨论文综合、冲突证据、数值/表格/公式回答、长答案、跨语言问题和更广的 no-answer 类型。
3. 对 Evidence Judge 的 8 个 false negatives 做 retrieval miss 与 conservatism 分解，避免本可回答的问题在 generation gate 前被拒绝。
4. 独立设计 claim extraction 与 claim-evidence alignment；每个 claim 必须保留 answer span 和 candidate citation provenance。
5. 实现 Citation Audit，分别评估 entailment、citation completeness、citation correctness 和 unsupported claims，不复用 benchmark labels 作为推理输入。
6. Citation Audit 验收后再设计 Agent workflow、retry/retrieval expansion 和 Evidence Graph；不得让 Agent 绕过 sufficiency gate。
7. 语料、embedding、Chroma、reranker、rewrite、Judge 或 Answer prompt/config 变化时，重新运行 Retrieval、Reranker、Query Rewrite、Evidence Sufficiency 和 Answer Generation 全部回归。

## 18. Development Documentation Rule
